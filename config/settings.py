...
class GatewayAuthConfig(BaseModel):
    """Inbound authentication for Hawkeye Gateway / Claude Desktop."""
    enabled: bool = Field(default=False, description="Enable secret-key gateway authentication")
    header_name: str = Field(default="X-Hawkeye-Key", description="Header containing the gateway secret")
    secret_key: Optional[SecretStr] = Field(default=None, description="Expected secret key for gateway validation")
    user_email_header: str = Field(default="X-User-Email", description="Header containing forwarded SSO user email")
...
def _apply_env_overrides(data: dict[str, Any]) -> dict[str, Any]:
    ...
    # Gateway authentication overrides
    if "gateway_auth" not in data:
        data["gateway_auth"] = {}
    if "GATEWAY_AUTH_ENABLED" in os.environ:
        data["gateway_auth"]["enabled"] = os.environ["GATEWAY_AUTH_ENABLED"].lower() in ("true", "1", "yes")
    if "GATEWAY_AUTH_HEADER_NAME" in os.environ:
        data["gateway_auth"]["header_name"] = os.environ["GATEWAY_AUTH_HEADER_NAME"]
    if "HAWKEYE_INTERNAL_SECRET" in os.environ:
        data["gateway_auth"]["secret_key"] = os.environ["HAWKEYE_INTERNAL_SECRET"]
    elif "GATEWAY_AUTH_SECRET_KEY" in os.environ:
        data["gateway_auth"]["secret_key"] = os.environ["GATEWAY_AUTH_SECRET_KEY"]
    if "GATEWAY_AUTH_USER_EMAIL_HEADER" in os.environ:
        data["gateway_auth"]["user_email_header"] = os.environ["GATEWAY_AUTH_USER_EMAIL_HEADER"]

    return data


def _expand_env_vars(val: Any) -> Any:
    """Recursively expand ${VAR} or ${VAR:-default} environment variables in config values."""
    ...
    if file_path and file_path.exists():
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            if isinstance(loaded, dict):
                raw_data = _expand_env_vars(loaded)
