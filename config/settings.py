...
class SecurityConfig(BaseModel):
    """Inbound SSO authentication via Microsoft Entra ID."""
    enable_auth: bool = Field(
        default=True,
        description="If false, bypasses JWT validation and assigns mock identity (dev mode)"
    )
    tenant_id: str = Field(default="your-azure-tenant-id", description="Microsoft Entra Directory (tenant) ID")
    client_id: str = Field(default="your-app-client-id", description="Entra Application (client) ID / audience")
    jwks_cache_ttl_seconds: int = Field(default=86400, description="Public JWKS key caching duration in seconds")
    accepted_issuers: List[str] = Field(
        default_factory=lambda: [
            "https://login.microsoftonline.com/{tenant_id}/v2.0",
            "https://sts.windows.net/{tenant_id}/",
        ],
        description="Accepted token issuer URL templates with {tenant_id} placeholder"
    )

    def get_formatted_issuers(self) -> List[str]:
        """Return accepted issuers with {tenant_id} resolved."""
        return [iss.format(tenant_id=self.tenant_id) for iss in self.accepted_issuers]
...
def _apply_env_overrides(data: dict[str, Any]) -> dict[str, Any]:
    ...
    # Entra ID / Security overrides
    if "AZURE_TENANT_ID" in os.environ:
        data["security"]["tenant_id"] = os.environ["AZURE_TENANT_ID"]
    if "AZURE_CLIENT_ID" in os.environ:
        data["security"]["client_id"] = os.environ["AZURE_CLIENT_ID"]
    if "SECURITY_ENABLE_AUTH" in os.environ:
        data["security"]["enable_auth"] = os.environ["SECURITY_ENABLE_AUTH"].lower() in ("true", "1", "yes")
    elif "ENABLE_AUTH" in os.environ:
        data["security"]["enable_auth"] = os.environ["ENABLE_AUTH"].lower() in ("true", "1", "yes")
