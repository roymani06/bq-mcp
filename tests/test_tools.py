...
Tests:
1. SQL Sanitizer (Regex keywords, BigQuery AST analysis, CTE mutation checks, query chaining).
2. Metadata TTLCache manager (SHA-256 key hashing, expiration, and caching decorator).
3. BigQuery client manager (guardrails, cost ceiling, pagination, type serialization).
4. Dynamic FastMCP tool registration matrix.
5. ASGI Streamable HTTP server & rate limiter middleware.
"""
...
from src.tools import _sanitize_message, build_mcp_server
...
class TestASGIServer:
    """Test suite for ASGI HTTP transport and healthcheck endpoints."""

    @pytest.fixture(autouse=True)
    def setup_server(self) -> None:
        self.settings = Settings(server=ServerConfig(endpoint_path="/mcp"))
    ...
    def test_mcp_endpoint_allows_access(self) -> None:
        with TestClient(self.app) as client:
            res = client.post(
                "/mcp",
...
class TestRateLimiting:
    ...
    def test_user_isolation_in_rate_limiting(self) -> None:
        ...
        with TestClient(app) as client:
            ...
            # Client 1 exhausts their 2 requests
            client.post("/mcp", headers={"X-Forwarded-For": "10.0.0.1"}, json=body)
            client.post("/mcp", headers={"X-Forwarded-For": "10.0.0.1"}, json=body)
            client1_blocked = client.post(
                "/mcp", headers={"X-Forwarded-For": "10.0.0.1"}, json=body
            )
            assert client1_blocked.status_code == 429

            # Client 2 must NOT be blocked despite Client 1 hitting the limit
            client2_res = client.post(
                "/mcp", headers={"X-Forwarded-For": "10.0.0.2"}, json=body
            )
            assert client2_res.status_code == 200
...
class TestConfigDrivenArchitecture:
    ...
    @pytest.mark.asyncio
    async def test_all_parameters_strictly_config_driven(self, tmp_path, monkeypatch) -> None:
        for env_var in [
            "SERVER_HOST", "SERVER_PORT", "SERVER_ENDPOINT_PATH", "SERVER_LOG_LEVEL",
            "SECURITY_ENABLE_AUTH", "ENABLE_AUTH", "AZURE_TENANT_ID", "AZURE_CLIENT_ID",
            "SERVICE_ACCOUNT_KEY_PATH", "BIGQUERY_PROJECT_ID", "BIGQUERY_LOCATION",
            "BIGQUERY_DEFAULT_ROWS_RETURNED", "BIGQUERY_MAX_ROWS_RETURNED",
            "BIGQUERY_MAX_BYTES_BILLED", "BIGQUERY_QUERY_TIMEOUT_SECONDS",
            "RATE_LIMIT_ENABLED", "RATE_LIMIT_REQUESTS_PER_MINUTE", "RATE_LIMIT_WINDOW_SECONDS",
            "CACHE_ENABLED", "CACHE_METADATA_TTL_SECONDS", "CACHE_MAX_CACHE_ENTRIES",
            "SANITIZER_ENABLED", "SANITIZER_MODE",
        ]:
