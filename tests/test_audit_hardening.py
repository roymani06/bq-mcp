...
class TestMalformedBearerHeader:
    """Verify server rejects non-Bearer auth schemes."""

    def test_basic_auth_header_rejected(self, rsa_key_pair, mock_jwks_client) -> None:
        private_key, _ = rsa_key_pair
        sec_cfg = SecurityConfig(enable_auth=True, tenant_id="t1", client_id="c1")
        validator = EntraTokenValidator(config=sec_cfg, jwks_client=mock_jwks_client)
        settings = Settings(
        ...
        with TestClient(app) as client:
            res = client.post("/mcp", headers={"Authorization": "Basic dXNlcjpwYXNz"})
...
class TestCORSPreflightBypass:
    """Verify OPTIONS requests don't require authentication."""

    def test_options_request_bypasses_auth(self, rsa_key_pair, mock_jwks_client) -> None:
        private_key, _ = rsa_key_pair
        sec_cfg = SecurityConfig(enable_auth=True, tenant_id="t1", client_id="c1")
        validator = EntraTokenValidator(config=sec_cfg, jwks_client=mock_jwks_client)
        settings = Settings(
        ...
        with TestClient(app) as client:
            ...
            # Should NOT return 401 — CORS preflight bypasses auth
            assert res.status_code != 401
